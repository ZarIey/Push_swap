...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ch_rotating_calcul.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ctardy <ctardy@student.42nice.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/10 17:46:33 by ctardy            #+#    #+#             */
/*   Updated: 2022/06/12 15:29:47 by ctardy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	all_in_b(t_prog *prog)
{
	...
	{
		ch_push_b(&prog->stack_a, &prog->stack_b);
		stack_a = prog->stack_a;
	}
...
{
	ch_double_rotate(prog, &rot_a, &rot_b);
	while (rot_a > 0)
	{
		rot_a--;
		ch_rotate_a(&prog->stack_a);
	}
	while (rot_a < 0)
	{
		rot_a++;
		ch_reverse_rotate_a(&prog->stack_a);
	}
	while (rot_b > 0)
	{
		rot_b--;
		ch_rotate_b(&prog->stack_b);
	}
	while (rot_b < 0)
	{
		rot_b++;
		ch_reverse_rotate_b(&prog->stack_b);
	}
}
